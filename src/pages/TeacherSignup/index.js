  ...
  const getContainerStyles = () => {
    if (isMobile) {
      return {
        ...
        contentClass: "flex flex-col items-center w-full h-full",
    ...
    <div className={styles.containerClass}>
      <div className={styles.cardClass}>
        <div className={styles.contentClass}>
          {/* Header: Logo + Title */}
          <div className="flex flex-col w-full text-center shrink-0" style={{ gap: isMobile ? '12px' : '16px' }}>
            <div>
              <div className="mb-2">
                <img
          ...
          {/* Form — flex-1 so it fills remaining card height for internal scroll */}
          <div className="flex flex-col w-full flex-1 min-h-0" style={{ alignItems: isMobile ? 'stretch' : 'center', marginTop: isMobile ? '20px' : '24px' }}>
            <TeacherSignupForm />
          </div>
        </div>
      </div>
